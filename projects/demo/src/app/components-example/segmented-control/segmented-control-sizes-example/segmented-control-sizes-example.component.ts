import { Component } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { IdsSegmentedControlDirective, IdsSegmentedControlItemComponent } from '@i-cell/ids-angular/segmented-control';
import { TranslateModule } from '@ngx-translate/core';

@Component({
  selector: 'app-segmented-control-sizes-example',
  imports: [
    FormsModule,
    IdsSegmentedControlDirective,
    IdsSegmentedControlItemComponent,
    TranslateModule,
  ],
  templateUrl: './segmented-control-sizes-example.component.html',
})
export class SegmentedControlSizesExampleComponent {
  public dense = 'week';
  public compact = 'week';
  public comfortable = 'week';
  public spacious = 'week';
}
